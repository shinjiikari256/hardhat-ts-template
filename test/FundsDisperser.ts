...
import { deploy, addresses } from "../common/for-tests";
...
describe("FundsDisperser", function () {
  ...
  describe("ETH", () => {
    ...
    it("✅ Получается отправить ETH", async function () {
      ...
      let tx = await disperseEther(receivers, amounts, totalAmount)
    ...
    it("✅ Избыток ETH возвращается отправителю", async function () {
      const { disperseEther, deployer, receivers } = await loadFixture(init);
      let tx = await disperseEther(receivers, amounts, 30n * power)
  ...
  describe("Tokens", () => {
    ...
    it("✅ Получается отправить токены", async function () {
      ...
      let tx = await disperseTokens(devToken, receivers, amounts)
